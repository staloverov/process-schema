...
import "./schema.sass";
import LineArrow, { ARROW_TYPE } from "src/components/line-arrow";
...
export default function Schema(props: SchemaProps) {
  ...
  const _scrollToTask = (taskId) => {
    if (horizontalProcess) {
      ...
      const _scrollLeft =
          _task.offset().left - _container.width() + _task.outerWidth() / 3,
        _scrollTop =
          _task.offset().top - _container.height() + _task.outerHeight() / 2;

      _scrollLeft > 1 && _container.scrollLeft(_scrollLeft); //todo think about it or not
      _container.scrollTop(_scrollTop);
    } else {
      setTimeout(() => {
        $("#js-task_" + taskId)[0].scrollIntoView({
          block: "center",
          inline: "center",
          behavior: "auto",
        });
  ...
  const addTaskWithLink = (taskId) => {
    if (props.onAddTaskWithLink) {
      props.onAddTaskWithLink(taskId);
    }
  };
  ...
  useEffect(() => {
    if (tree && !props.activeTaskId) {
      const _firstActiveTask = Object.values(tree.nodes).find(
        (task) => task.state !== TASK_STATE.DONE.value
      );
  ...
  const onClick = (e) => {
    ...
    if (
      !(
        e.target.closest(".process-schema__task") ||
        e.target.closest(".task__action-menu")
      )
    ) {
      setActiveTask(0);
    ...
    if (
      !(
        e.target.closest(".process-schema__x-arrow") ||
        e.target.closest(".x-arrow__tooltip")
      )
    ) {
      if (activeArrow) setActiveArrow(null);
    }
  ...
  return (
    <div className="process-body__schema" onMouseDown={onClick}>
      <h6 className="process-schema__title">
        <span>Схема процесса</span>
        <button
          className="process-schema__rotate-button"
      ...
      <Xwrapper>
        <div
          className="process-schema__canvas-background _with-custom-scroll"
          id="schema_container"
          ref={canvas}
        >
      ...
      <button
        className="process-schema__add-task-button orange-button small-button"
        onClick={_onAdd}
      >
        Добавить задачу
      </button>
